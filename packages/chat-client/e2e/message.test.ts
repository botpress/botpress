...
const checkApiCanSendAndReceiveMessages = async (
  ...
  listener.once('error', (err) => {
    console.error('An error occured in conversation', err)
    throw err
  })

  listener.on('unknown', (ev) => {
    console.warn('Received an unknown event', ev)
  })
...
test.only('api allows receiving bloc messages from bot', async () => {
  ...
  const responsePayload = await checkApiCanSendAndReceiveMessages(
    {
      client,
      conversationId,
    },
  ...
  expect(responsePayload.type).toEqual('block')
