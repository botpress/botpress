...
const _anyOfFallbackExtractor = (
  /** Every key should be preceeded with a "?." | TODO: Rewrite this */
  valueExtractor: (value: any) => string,
  options: { pathBackoff?: number } = {}
): FallbackExtractor => {
  const { pathBackoff = 0 } = options

  if (pathBackoff < 0 || !Number.isInteger(pathBackoff)) {
    throw new Error('Path backoff must be a non-negative integer')
  }

  const backoffBy = -1 - pathBackoff
  return (failedPath, jsonPathExtractor) => {
    const newPath = `$.${failedPath.slice(0, backoffBy).join('.')}.anyOf[*]`
    const match = jsonPathExtractor(newPath).find(({ value }) => typeof valueExtractor(value) === 'string')
    return match ? { value: valueExtractor(match.value), path: match.resolvedPath } : null
  }
}

export const titleFallbackExtractor: FallbackExtractor = _anyOfFallbackExtractor(
  (value) => value?.['x-zui']?.title ?? '',
  { pathBackoff: 1 }
)

export const descriptionFallbackExtractor: FallbackExtractor = _anyOfFallbackExtractor(
