...
export class ApiClient {
  ...
  public async findPrivateIntegration(ref: IntegrationRef): Promise<Integration | undefined> {
    if (ref.type === 'id') {
      return this.validateStatus(() => this.client.getIntegration(ref).then((r) => r.integration), [404, 400])
    }
    return this.validateStatus(() => this.client.getIntegrationByName(ref).then((r) => r.integration), [404, 400])
  }

  public async findPublicIntegration(ref: IntegrationRef): Promise<Integration | undefined> {
    if (ref.type === 'id') {
      return this.validateStatus(() => this.client.getPublicIntegrationById(ref).then((r) => r.integration), [404, 400])
    }
    return this.validateStatus(() => this.client.getPublicIntegration(ref).then((r) => r.integration), [404, 400])
  }
