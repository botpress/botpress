...
const files = glob.sync(path.join(bpDir, 'src/core/config/*.ts'))
const program = getProgramFromFiles(files)

const writeSchema = (typeName: string, jsonFile: string) => {
  const definition = generateSchema(program, typeName, settings)
  const json = JSON.stringify(definition, null, 2) + os.EOL + os.EOL

  const fileToWrite = path.resolve(bpDir, 'dist/core/config/schemas', jsonFile)
