/**
 * This file is copied from "packages/cli/src/utils/case-utils.ts"
 * TODO: extract in common package
 */
...
export const to = utils.mapValues(fromTokens, (fn) => (text: string) => {
  let tokens = splitTokens([text])
  tokens = filterTokens(tokens)
  return fn(tokens)
}) satisfies Record<SupportedCase, (text: string) => string>

export const is = utils.mapValues(to, (fn) => (text: string) => {
