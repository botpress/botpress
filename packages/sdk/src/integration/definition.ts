...
type ActionDefinition<TAction extends BaseActions[string]> = {
  ...
  output: SchemaDefinition<AnyZodObject> // both input and output cannot be inferred by typescript
}
...
type EntityDefinition<
  TEntity extends BaseEntities[string],
  TEvents extends BaseEvents = BaseEvents,
  TActions extends BaseActions = BaseActions
> = SchemaDefinition<TEntity> & {
  title?: string
  description?: string
  actions?: Partial<Record<EntityOperation, keyof TActions>>
  events?: Partial<Record<EntityEvent, keyof TEvents>>
}
...
> = {
  ...
  entities?: {
    [K in keyof TEntities]: EntityDefinition<TEntities[K], TEvents, TActions>
  }
}
