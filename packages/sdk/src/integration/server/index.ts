...
/**
 * Thrown when an integration has no handler registered for a channel that is part of its own schema.
 *
 * This is a server-side bug (a misconfigured or misdeployed integration), not something the caller can
 * fix by retrying - conceptually this should be a 501, but the runtime's error pipeline only knows how
 * to serialize the error classes generated by the BP API, which don't include a 501. We reuse
 * `InvalidPayloadError` for that reason, but keep it as a distinct, `instanceof`-checkable class.
 */
class ChannelHandlerNotFoundError extends InvalidPayloadError {
  public constructor(channel: string) {
    super(`Channel "${channel}" has no handler registered`)
  }
}
...
const onMessageCreated = async ({ ctx, req, client, logger, instance }: ServerProps) => {
  ...
  if (!channelHandler) {
    throw new ChannelHandlerNotFoundError(conversation.channel)
  }
...
const onUpdateConversation = async ({ ctx, req, client, logger, instance }: ServerProps) => {
  ...
  if (!channelHandler) {
    throw new ChannelHandlerNotFoundError(currentConversation.channel)
  }
