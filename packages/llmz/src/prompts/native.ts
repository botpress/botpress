...
function runtimeRules(props: LLMzPrompts.InitialStateProps, chat: boolean): string {
  ...
  rules.push(
    ...
    '# Recovery',
    'Execution reports identify completed actions, inspected values, memory changes, and errors. Reuse preserved variables and completed actions; retry only failed work.',
  ...
  if (hasExits) {
    rules.push(
      'Use a failure exit only when recovery is unavailable, unsafe, forbidden, its budget is exhausted, or task instructions make the failure terminal. Do not select failure in advance for an unseen error.'
    )
  }
