...
Recovery feedback uses XML-style sections: `<error>` contains the specific error code and readable message, `<stack_trace>` shows source-mapped guest code with line numbers and carets, and `<recovery>` explains how to continue. Tool calls, sent messages, memory changes, and inspection results each have their own section. Content containing XML characters is enclosed in CDATA so code examples, entity literals, and retrieval tags stay unchanged without closing a section. Validation messages preserve line breaks and the expected TypeScript shape.
...
A `ThinkSignal` report separates its reason, guest interruption site (when available), and context. It tells the model to review that context and continue from retained values and acknowledged effects. It does not resume the suspended JavaScript stack.
