...
| `MISSING_CHAT_RESPONSE`   | `MissingChatResponseError`  | Recoverable. Chat tried to complete through `listen` while the assistant was not the last participant to speak; disabled by `options.requireChatResponse: false`.                                                 |
