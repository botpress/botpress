...
| ------------------------------ | ------------------------------------ | ------------------------------- |
| Who yields components          | Returned `chat.present` decision     | Tool handler itself             |
| Tool handler type              | `async (input) => output`            | `async function* (input)`       |
| LLM needs to know components   | Yes — documented VM presentation API | No — tool handles it internally |
| Progress updates mid-execution | `await chat.send(...)`               | `yield` between steps           |
