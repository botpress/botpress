...
import { sanitizeUtterances } from './pipelines/intents/tokens_utils'
...
export class RemoteLanguageProvider implements LanguageProvider {
  ...
  async primeLanguageCaches(intentDefs: sdk.NLU.IntentDefinition[], lang: string): Promise<void> {
    const utterances = _.flatten(intentDefs.map(intent => sanitizeUtterances(intent.utterances[lang])))
    const tokens = await this.tokenize(utterances, lang)
    await this.vectorize(_.flatten(tokens), lang)
  }
