...
export default async (bp: typeof sdk, state: NLUState) => {
  ...
  const scheduleSyncNLU = (botId: string) => {
    ...
    syncByBots[botId] = setTimeout(() => {
      delete syncByBots[botId]
      const botEngine = state.nluByBot[botId] as ScopedEngine
  ...
  router.get('/currentModelHash', async (req, res) => {
    const engine = state.nluByBot[req.params.botId] as ScopedEngine
    if (engine.modelHash) {
  ...
  router.get('/confusion/:modelHash/:version', async (req, res) => {
    const engine = state.nluByBot[req.params.botId] as ConfusionEngine
  ...
  router.get('/confusion', async (req, res) => {
    try {
      ...
      const confusions = await (state.nluByBot[botId] as ScopedEngine).storage.getAllConfusionMatrix()
  ...
  router.post('/confusion', async (req, res) => {
    try {
      const botEngine = state.nluByBot[req.params.botId] as ScopedEngine
  ...
  router.get('/intents', async (req, res) => {
    res.send(await (state.nluByBot[req.params.botId] as ScopedEngine).storage.getIntents())
  })

  router.get('/intents/:intent', async (req, res) => {
    res.send(await (state.nluByBot[req.params.botId] as ScopedEngine).storage.getIntent(req.params.intent))
  })
  ...
  router.get('/contexts', async (req, res) => {
    const botId = req.params.botId
    const intents = await (state.nluByBot[botId] as ScopedEngine).storage.getIntents()
  ...
  router.get('/entities', async (req, res) => {
    const entities = await (state.nluByBot[req.params.botId] as ScopedEngine).storage.getAvailableEntities()
    res.json(entities)
  })
  ...
  router.post('/extract', async (req, res) => {
    ...
    try {
      const result = await state.nluByBot[req.params.botId].extract(eventText.preview, [], [])
      res.send(result)
    } catch (err) {
