...
export default class LuisProvider extends Provider {
  private appId: any
  private programmaticKey: any
  private appSecret: any
  private appRegion: any
  private syncingSince: Date
  ...
  async deleteVersion() {
    try {
      const del = await axios.delete(
        `https://${this.appRegion}.api.cognitive.microsoft.com/luis/api/v2.0/apps/${
          this.appId
        }/versions/${LUIS_APP_VERSION}/`,
        {
          headers: {
            'Ocp-Apim-Subscription-Key': this.programmaticKey
          }
  ...
  async sync() {
    // FIXME: Sly what are we doing here?
    ...
    try {
      const result = await axios.post(
        `https://${this.appRegion}.api.cognitive.microsoft.com/luis/api/v2.0/apps/${
          this.appId
        }/versions/import?versionId=${LUIS_APP_VERSION}`,
        body,
        {
          headers: {
            'Ocp-Apim-Subscription-Key': this.programmaticKey
  ...
  async train() {
    let res = await axios.post(
      `https://${this.appRegion}.api.cognitive.microsoft.com/luis/api/v2.0/apps/${
        this.appId
      }/versions/${LUIS_APP_VERSION}/train`,
      {},
      {
        headers: {
          'Ocp-Apim-Subscription-Key': this.programmaticKey
