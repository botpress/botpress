...
export default async (bp: typeof sdk, bots: ScopedBots) => {
  ...
  router.get('/:topicName/questions', asyncMiddleware(async (req: Request, res: Response) => {
  ...
  router.post('/:topicName/questions', asyncMiddleware(async (req: Request, res: Response, next: Function) => {
    const { storage } = bots[req.params.botId]
  ...
  router.get('/:topicName/questions/:id', asyncMiddleware(async (req: Request, res: Response) => {
    const { storage } = bots[req.params.botId]
    const items = await storage.fetchItems(req.params.topicName)
  ...
  router.post('/:topicName/questions/:id', asyncMiddleware(async (req: Request, res: Response, next: Function) => {
    const { storage } = bots[req.params.botId]
  ...
  router.post('/:topicName/questions/:id/delete', asyncMiddleware(async (req: Request, res: Response) => {
    const { storage } = bots[req.params.botId]
  ...
  router.post('/:topicName/actions/:id', asyncMiddleware(async (req: Request, res: Response) => {
    const { storage } = bots[req.params.botId]
    const items = await storage.fetchItems(req.params.topicName)
    const item = items.find(x => x.id === req.params.id)
  ...
  router.get('/:topicName/export', asyncMiddleware(async (req: Request, res: Response) => {
    const { storage } = bots[req.params.botId]
  ...
  router.post('/:topicName/import', multer().single('file'), asyncMiddleware(async (req: any, res: Response) => {
  ...
  router.get('/questionsByTopic', asyncMiddleware(async (req: Request, res: Response) => {
    const { storage } = bots[req.params.botId]
