...
const onElementChanged = async (bp, botId, action, element) => {
  ...
  if (action === 'delete') {
    const qnaStorage = await botScopedStorage.get(botId)
    const questions = await qnaStorage.getQuestions({ question: '', categories: [] }, { limit: 0, offset: 0 })
    const affectedItems = questions.items.filter(
      item => item.data.answers.filter(answer => answer.contentId === element.id).length
    )
    for (const item of affectedItems) {
      ...
      if (newAnswers.length) {
        ...
        await qnaStorage.update({ ...item.data, answers: ['output responses deleted'], enabled: false }, item.id)
      }
    }
  }
}
