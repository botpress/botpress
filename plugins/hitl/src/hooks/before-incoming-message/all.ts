...
export const handleMessage: bp.HookHandlers['before_incoming_message']['*'] = async (props) => {
  const conversation = await props.conversations.getById({
...
const _handleDownstreamMessage = async (
  ...
  const upstreamConversation = await props.conversations.getById({ id: upstreamConversationId })
...
const _handleUpstreamMessage = async (
  ...
  const downstreamConversation = await props.conversations.getById({ id: downstreamConversationId })
  const downstreamCm = conv.ConversationManager.from(props, downstreamConversation)
