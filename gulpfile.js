...
if (yn(process.env.GULP_PARALLEL)) {
  gulp.task('build', gulp.series([core.build(), ui.initStudio, gulp.parallel(modules.build(), ui.build())]))
} else {
  gulp.task('build', gulp.series([core.build(), ui.initStudio, modules.build(), ui.build()]))
}
