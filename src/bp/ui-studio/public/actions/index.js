...
export const fetchContentCategories = () => dispatch => axios.get('/api/content/categories').then(({ data }) => {
  dispatch(receiveContentCategories(data));
});

export const receiveContentItems = createAction('CONTENT/ITEMS/RECEIVE');
export const fetchContentItems = ({ id, from, count, searchTerm }) => dispatch => axios.get(`/api/content/items`, { params: { categoryId: id, from, count, searchTerm } }).then(({ data }) => dispatch(receiveContentItems(data)));

export const receiveContentItemsRecent = createAction('CONTENT/ITEMS/RECEIVE_RECENT');
export const fetchContentItemsRecent = ({ searchTerm, count = 5, categoryId = 'all' }) => dispatch => axios.get(`/api/content/items`, { params: { categoryId, count, searchTerm, orderBy: ['createdOn', 'desc'] } }).then(({ data }) => dispatch(receiveContentItemsRecent(data)));

const getBatchedContentItems = ids => axios.get(`/api/content/items-batched/${ids.join(',')}`).then(({ data }) => data.reduce((acc, item, i) => {
  acc[ids[i]] = item;
  return acc;
}, {}));
...
export const fetchContentItem = (id, { force = false, batched = false } = {}) => (dispatch, getState) => {
  ...
  return (batched ? getBatchedContentItem(id) : getSingleContentItem(id)).then(data => dispatch(receiveContentItem(data)));
};

export const receiveContentItemsCount = createAction('CONTENT/ITEMS/RECEIVE_COUNT');
export const fetchContentItemsCount = (categoryId = 'all') => dispatch => axios.get(`/api/content/items/count`, { params: { categoryId } }).then(data => dispatch(receiveContentItemsCount(data)));

export const upsertContentItem = ({ categoryId, formData, modifyId }) => () => axios.post(`/api/content/categories/${categoryId}/items/${modifyId || ''}`, { formData });
