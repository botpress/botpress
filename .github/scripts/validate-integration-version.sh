...
if [ -z "$integration" ]; then
  echo "::error::Integration name is not provided."
  exit 1
fi
...
if ! public_versions=$(jq -r '.[] | select(.public) | .version' <<< "$integrations_json"); then
  echo "::error::Failed to parse deployed versions for integration $integration."
  exit 1
fi
...
if ! [[ "$version" =~ ^([0-9]+)\.([0-9]+)\.([0-9]+)$ ]]; then
  echo "::error::Integration $integration has invalid version $version."
  exit 1
fi
...
if [ -z "$latest_version" ]; then
  if [ "$minor" -ne 0 ] || [ "$patch" -ne 0 ]; then
    echo "::error::New integration $integration must start at x.0.0, not $version."
    exit 1
  fi
  exit 0
fi
...
echo "::error::Keep $latest_version for a non-breaking change, or use $((latest_major + 1)).0.0 for a breaking change."
exit 1
