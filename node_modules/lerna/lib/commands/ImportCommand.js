...
var ImportCommand = function (_Command) {
  ...
  _createClass(ImportCommand, [{
    ...
    value: function initialize(callback) {
      ...
      var targetDir = _path2.default.join(targetBase, externalRepoBase);

      // Compute a target directory relative to the Git root
      var gitRepoRoot = _GitUtilities2.default.getWorkspaceRoot(this.execOpts);
      var lernaRootRelativeToGitRoot = _path2.default.relative(gitRepoRoot, this.repository.rootPath);
      this.targetDirRelativeToGitRoot = _path2.default.join(lernaRootRelativeToGitRoot, targetDir);

      if (_FileSystemUtilities2.default.existsSync(_path2.default.resolve(this.repository.rootPath, targetDir))) {
      ...
      this.logger.info("", `About to import ${this.commits.length} commits from ${inputPath} into ${targetDir}`);
    ...
    value: function createPatchForCommit(sha) {
      ...
      var formattedTarget = this.targetDirRelativeToGitRoot.replace(/\\/g, "/");
