## Modules

<!-- * [How to use modules](how-to-use-modules.md) -->
* [How to create a module](how-to-create-a-module.md)
    * [Modules Guidelines](modules-guidelines.md)
    * [How do I deploy a module to the community?](how-to-deploy-a-module.md)
    * [How to configure my module](how-to-configure-my-module.md)
    * [Which APIs should my module expose?](which-apis-should-my-module-expose.md)
    * [When should I use the Events vs APIs?](when-should-i-use-the-events-vs-apis.md)
    * [Should I create a UI for my module?](should-i-create-a-ui-for-my-module.md)
    * [How can I change the icon of my module and its name in the menu?](how-to-change-the-icon-and-name.md)
    * [How do I write documentation for my module?](how-to-write-documentation.md)