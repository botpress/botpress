...
Although a typical pipeline would have 3 stages (e.g: _Development_, _Staging_ and _Production_) where a bot goes through all of them Botpress let you define your own stages on a single spreaded across multiple Botpress deployement.
...
Want to lock a bot in a particular stage or change it's name along the pipeline ? You can do so using available hooks
