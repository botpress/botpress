...
You can then store the user's choice in the `state` by preparing a [simple action](/docs/main/code) for this purpose. Let's assume we offer the choice between English and Arabic, after the user chooses their language, we will set `state.language` either to "En" or "Ar".

## Adopting content schema

Botpress allows you to define a [custom content type](/docs/main/content) that will allow you to store text in multiple languages. Here's an example of a `translate_text` content-type:
