...
function renderElement(data, channel) {
  ...
  const style = language === 'Ar' ? 'direction: rtl;' : ''
  const text = `<div style="${style}">${data[`text${language}`]}</div>`
  return [ { type: 'text', typing: true, markdown: true, text } ]
}
...
module.exports = {
  ...
  jsonSchema: {
    ...
    properties: {
      ...
      textAr: { type: 'string', title: 'Text (Arabic)', classNames: 'rtl' }
    }
  },
  uiSchema: {},
