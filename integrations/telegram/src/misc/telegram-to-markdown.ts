...
export function applyMarksToText(text: string, marks: TelegramMark[], options: ApplyMarksOptions = {}) {
  const defaultHandlers: Record<string, MarkHandler> = {
    bold: (text: string, _: Record<string, unknown>) => `**${text}**`,
    italic: (text: string, _: Record<string, unknown>) => `__${text}__`,
    strikethrough: (text: string, _: Record<string, unknown>) => `~~${text}~~`,
    code: (text: string, _: Record<string, unknown>) => `\`${text}\``,
    text_link: (text: string, data: Record<string, unknown>) => `[${text}](${data?.url || '#'})`,
    spoiler: (text: string, _: Record<string, unknown>) => `||${text}||`,
    pre: (text: string, data: Record<string, unknown>) => `\`\`\`${data?.language || ''}\n${text}\n\`\`\``,
    blockquote: (text: string, _: Record<string, unknown>) => `> ${text}`,
    phone_number: (text: string, _: Record<string, unknown>) => {
      return `[${text}](tel:${text.replace(/\D/g, '')})`
    },
    email: (text: string, _: Record<string, unknown>) => {
  ...
  // Merge custom handlers with defaults (I will likely remove this since I can't think of a reason to keep it)
  const handlers = { ...defaultHandlers, ...options.handlers }

  // Validate and sort marks by offset (descending) to avoid position shifts (Generated by ChatGPT, likely unnecessary w/TypeScript types)
  const validMarks = marks
    .filter((mark) => {
      if (!mark || typeof mark !== 'object') return false
      if (mark.offset < 0 || mark.length <= 0) return false
      return mark.offset + mark.length <= text.length
    })
    .sort((a, b) => b.offset - a.offset)

  for (const mark of validMarks) {
    const { type, offset, length, url, language } = mark
    ...
    const markedText = text.substring(offset, offset + length)
    const afterText = text.substring(offset + length)

    const transformedText = handler(markedText, {
      url,
      language,
    })

    text = beforeText + transformedText + afterText
  }
