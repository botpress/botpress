...
export async function generateOutgoingMessage({
  ...
  try {
    ...
    if (dimensions?.width === 512 && dimensions.height === 512 && isUnderSizeLimit) {
      ...
      logger.forBot().warn(`Image is too big for a sticker. Current size is ${buffer.length} bytes. Must be smaller than ${512 * 1024} bytes`);
      return undefined
    } else {
      logger.forBot().warn(`Image does not meet WhatsApp size requirements. Current size is ${dimensions?.width} wide by ${dimensions?.height} high. Expected dimensions are 512 x 512`);
      return undefined
    }
  } catch (error) {
