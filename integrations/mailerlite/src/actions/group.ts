...
export const listGroups: bp.Integration['actions']['listGroups'] = async ({ client, ctx, input, logger }) => {
  ...
  return (
    parsed ?? {
      ...
      meta: { current_page: 1, from: 0, last_page: 1, links: [], path: '', per_page: 0, to: 0, total: 0 },
    }
  )
}

export const assignToGroup: bp.Integration['actions']['assignToGroup'] = async ({ client, ctx, input, logger }) => {
  const mlClient: MailerLiteClient = await getAuthenticatedMailerLiteClient({ ctx, client })
