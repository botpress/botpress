...
const listGroups: ActionDefinition = {
  ...
  input: {
    schema: z.object({
      limit: z
        .number()
        .title('Max number of groups to show')
        .describe('Limits the number of groups that the API returns')
        .optional(),
...
const unassignFromGroup: ActionDefinition = {
  ...
  output: {
    schema: z.object({
      success: z
        .boolean()
        .title('Success of unassignment')
        .describe('Boolean representing the success of the unassignment operation'),
      message: z
        .string()
        .title('Return message for more description')
        .describe('Description of success of the unassignment operation'),
    }),
  },
}
