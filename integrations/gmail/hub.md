...
The Gmail integration provides a comprehensive set of actions that allow your chatbot to interact with Gmail programmatically. These actions enable you to manage emails, send messages, organize your inbox, and handle attachments.

### Message Management Actions

#### `listMessages`

Search and list email messages from your Gmail inbox.

- **Input parameters:**
  - `query` (optional): Gmail search query string (e.g., "from:example@gmail.com", "subject:meeting")
  - `maxResults` (optional): Maximum number of messages to return
  - `pageToken` (optional): Token for pagination
- **Output:** List of message IDs and thread IDs, along with pagination information

#### `getMessage`

Retrieve full details of a specific email message by its ID.

- **Input parameters:**
  - `id`: The message ID
- **Output:** Complete message details including headers, body, labels, snippet, and metadata
...
Move an email message to the trash (soft delete).
...
- **Input parameters:**
  - `id`: The message ID to restore
- **Output:** Success status

#### `changeMessageLabels`

Modify labels on an email message (e.g., mark as read/unread, archive, add custom labels).

- **Input parameters:**
  - `id`: The message ID
  - `addLabelIds` (optional): Array of label IDs to add
  - `removeLabelIds` (optional): Array of label IDs to remove
- **Output:** Success status
...
Alternative method to retrieve an attachment from a mail message.
...
### Email Composition and Sending Actions

#### `sendMail`

Send an email using a raw email format.

- **Input parameters:**
  - `raw`: The raw email content in RFC 2822 format (base64 encoded)
  - `threadId` (optional): Thread ID if replying to an existing conversation
- **Output:** Sent message details including message ID, thread ID, and label IDs

#### `composeRawEmail`

Compose a raw email in the proper format for sending.

- **Input parameters:**
  - `to`: Recipient email address
  - `subject`: Email subject line
  - `body`: Email body content
- **Output:** Raw email string in RFC 2822 format (ready to be used with `sendMail`)
...
These actions can be used in your chatbot workflows to automate email management tasks such as:

- Searching and retrieving emails based on criteria
- Organizing emails by adding or removing labels
- Sending automated email responses
- Managing attachments
- Cleaning up your inbox by deleting or trashing messages
