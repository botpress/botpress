...
export const addCommentToTask: Implementation['actions']['addCommentToTask'] =
  async ({ ctx, input, logger }) => {
    ...
    let response
    try {
      response = await asanaClient.addCommentToTask(
        validatedInput.taskId,
        validatedInput.comment
      )
      logger
        .forBot()
        .info(`Successful - Add Comment to Task - ${response.text}`)
    } catch (error) {
      logger
        .forBot()
        .debug(`'Add Comment to Task' exception ${JSON.stringify(error)}`)
      response = {}
    }
