...
export const createOrRetrieveCustomer: Implementation['actions']['createOrRetrieveCustomer'] =
  async ({ ctx, logger, input }) => {
    ...
    try {
      ...
      if (customers.length === 0) {
        const params = {
          ...
          address: validatedInput.address
            ? JSON.parse(validatedInput.address)
            : undefined,
        }
        ...
        response =
          customers.length === 1 ? { customer: customers[0] } : { customers }
      }
      logger
        .forBot()
        .info(
          `Successful - Create or Retrieve Customer ${
      ...
      logger
        .forBot()
        .debug(
          `'Create or Retrieve Customer' exception ${JSON.stringify(error)}`
        )
    }
