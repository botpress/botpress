...
export const createPaymentLink: Implementation['actions']['createPaymentLink'] =
  async ({ ctx, logger, input }) => {
    ...
    try {
      ...
      if (!price) {
        price = await StripeClient.createPrice(
          product.id,
      ...
      logger
        .forBot()
        .info(`Successful - Create Payment Link - ${paymentLink.id}`)
    } catch (error) {
      response = {}
      logger
        .forBot()
        .debug(`'Create Payment Link' exception ${JSON.stringify(error)}`)
    }
