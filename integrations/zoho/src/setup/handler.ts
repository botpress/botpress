...
const DATA_CENTER_CHOICES: { label: string; value: DataCenter }[] = [
  { label: 'US - accounts.zoho.com', value: 'us' },
  { label: 'EU - accounts.zoho.eu', value: 'eu' },
  { label: 'IN - accounts.zoho.in', value: 'in' },
  { label: 'AU - accounts.zoho.com.au', value: 'au' },
  { label: 'CN - accounts.zoho.com.cn', value: 'cn' },
  { label: 'JP - accounts.zoho.jp', value: 'jp' },
  { label: 'CA - accounts.zohocloud.ca', value: 'ca' },
]
...
export const handler: Handler = async (props) => {
  ...
  try {
    ...
    props.logger.forBot().error('Error in Zoho OAuth flow:', message)
    return generateRedirection(oauthWizard.getInterstitialUrl(false, message))
  }
}
