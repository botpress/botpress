...
4. Copy the legacy Bot Token (starts with `xoxb-`) or Refresh Token (starts with `xoxe-1-`) and paste it into the integration settings in Botpress. Initially, your Slack app will have a legacy token (`xoxb-`), but after the initial authorization (when you press "Save Configuration"), it will be exchanged for a Refresh Token (`xoxe-1`). The refresh token will automatically be added to your configuration, so you should not need to add anything after the initial Bot Token (`xoxb-1`). If you need to access your Refresh Token for any reason after the initial authorization, you will be able to see it in your Slack API portal.
