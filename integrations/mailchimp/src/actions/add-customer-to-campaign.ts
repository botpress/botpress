...
export const addCustomerToCampaign: IntegrationProps['actions']['addCustomerToCampaign'] = async ({
  ctx,
  input,
  logger,
}) => {
  ...
  try {
    ...
    let logMessage
    if (!isAlredyAdd) {
      response = await mailchimpClient.addCustomerToCampaignList(validatedInput.campaignId, customer)
    } else {
