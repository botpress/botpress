...
export const addCustomerToList: IntegrationProps['actions']['addCustomerToList'] = async ({ ctx, input, logger }) => {
  ...
  let response
  try {
    const isAlredyAdd = await mailchimpClient.checkIfCustomerInList(validatedInput.listId, validatedInput.email)
    let logMessage
    if (!isAlredyAdd) {
      response = await mailchimpClient.addCustomerToList(validatedInput.listId, customer)
    ...
    logger.forBot().debug(`'Add Customer To List' exception ${JSON.stringify(error)}`)
    response = {}
  }
