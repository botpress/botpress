...
test.each(Object.entries(stripAllTests))(
  ...
  (_testName: string, testValues: { input: string; expected: string }): void => {
    const actual = transformMarkdownForTwilio(testValues.input, 'sms/mms')
...
test.each(Object.entries(stripAllTests))(
  '[RCS] Test %s',
  (_testName: string, testValues: { input: string; expected: string }): void => {
    const actualRcs = transformMarkdownForTwilio(testValues.input, 'rcs')
...
test('[SMS, MMS] Multi-line multi markup test', () => {
  const actual = transformMarkdownForTwilio(bigInput, 'sms/mms')
  expect(actual).toBe(expectedForBigInputSMS)
})

test('[RCS] Multi-line multi markup test', () => {
  const actualRcs = transformMarkdownForTwilio(bigInput, 'rcs')
