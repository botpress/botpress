...
export const processMessage = async (message: ChangeNotification, ctx: IntegrationContext, client: Client) => {
  ...
  if (message.lifecycleEvent) {
    ...
    const notificationContent = notificationContentSchema.parse(await graphClient.getNotificationContent(odataId))
    ...
    if (notificationContent.from?.emailAddress?.address === ctx.configuration.emailAddress) {
      console.warn('invalid email, the email should not send itself, avoiding recursion error.')
      return
    }
